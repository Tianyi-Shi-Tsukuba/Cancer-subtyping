...
mvsc <- function(x, k, sigmas = NULL, neighbours = NULL, clustering = TRUE)
{
  ...
  for(i in 1:length(x))
  {
      ...
      # view.dist <- as.matrix(stats::dist(x[[i]]))
      # view.dist <- as.matrix(SC_cSNN(x[[i]], 5, 1.5))
      view.dist <- as.matrix(SC_nSNN(x[[i]], 5, 1.5))
      # view.dist <- as.matrix(adaptivesmatrix(x[[i]], 5))
      # view.dist <- as.matrix(keradaptivesmatrix(x[[i]], 5, 1.5))
      # view.dist <- as.matrix(knnsmatrix(x[[i]], 5, 1.5))
      # view.dist <- as.matrix(scstsmatrix(x[[i]], 5))
      view.dist=view.dist/3
      write.csv(view.dist,"view.dist.csv")
